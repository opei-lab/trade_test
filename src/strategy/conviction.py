...
CONVICTION_CHECKS = [
    ...
    # === 需給構造（バックテスト実証済み） ===
    {
        ...
        "weight": 5,
        "category": "需給",
        "description": "深底値+出来高1.2-2.0倍（急増せず静かに増加=大口の仕込みパターン。勝率85%）",
        "check": lambda ctx: ctx["supply"].get("price_position", 50) < 15 and 1.2 <= ctx["supply"].get("volume_anomaly", 0) <= 2.0,
    },
    {
        "id": "deep_bottom_penny",
        "name": "超低位+深底値",
        "weight": 5,
        "category": "需給",
        "description": "¥300以下+底値10%以下（勝率88%、平均+58.5%の最強パターン）",
        "check": lambda ctx: ctx["supply"].get("price_position", 50) < 10 and 0 < ctx.get("market_cap", 999e9) and ctx.get("current_price", 9999) < 300,
    },
    {
        "id": "vol_up_and_dry",
        "name": "出来高増+売り枯れ",
